...
TARGET="$HOME/.local/bin/sessions"

case "${1:-}" in
  install)
    mkdir -p "$(dirname "$TARGET")"
    ln -sfn "$SCRIPT_DIR/sessions.sh" "$TARGET"
    echo "Linked: $TARGET → $SCRIPT_DIR/sessions.sh"
    ;;
  uninstall)
    if [ -L "$TARGET" ]; then
      rm "$TARGET"
    ...
    ;;
  status)
    if [ -L "$TARGET" ] && [ "$(readlink -f "$TARGET")" = "$SCRIPT_DIR/sessions.sh" ]; then
