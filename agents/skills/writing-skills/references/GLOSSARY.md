...
Executable code in a skill's `scripts/` directory that replaces a prose-described procedure with a single deterministic call. The strongest lever on **predictability**: prose **steps** are re-derived by the model on every run — improvised commands, improvised order, variance — while a script runs identically every time, removing the model from the loop for that step and converging probabilistic output to determinism. Reach for one wherever a repeatable CLI procedure exists, whether it gathers context (inspect state, list targets, diff environments) or executes a change (migrate, link, scaffold) — and word the step to run the script, never to describe what it does inside. The division of labour: mechanics belong in scripts, prose is spent on judgment calls. A script should solve, not punt — handle its own edge cases, document its dependencies, and fail with an error that tells the agent what to do next, so no half of the procedure falls back to improvisation.

_Avoid_: tool, helper, utility, automation
